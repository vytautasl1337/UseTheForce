...
from functions.dictionary_values.addValues_old import addValues
from functions.dictionary_values.addValues_old import createGroupDictionary
...
def getVar_group(self,folder_group):
    ...
    try:
        columns_to_check = ['id','data','delay','group','gender','age','calibration']
    ...
    save_destination = os.path.join(folder_group,'Results/Group_output_%s'%str(datetime.datetime.now().strftime('%Y%m%d%H%M') ))
    ...
    print('--------------------------------------')
    # Check groups and produce dictionary for each group and hand
    curve_output,slope_output = createGroupDictionary(question_table)
    ...
    for subject in range(len(question_table)):
        ...
        if question_table['data'][subject]==1:
            ...
            try:
                ...
                with PdfPages(os.path.join(sub_destination,'{}_responses.pdf'.format(question_table['id'][subject]))) as pdf: 
                        ...
                        try:
                            ...
                            for event in range(blocks_to_take*evntBlc):
                                ...
                                if corr_run[0][event] == 1:
                                    # Get indexes of the events AND CORRECT FOR 60ms DELAY IN THE SCANNER SET EXPERIMENT AS 1
                                    ...
                                    if grip_run[0][event] == 1 or grip_run[0][event] == -1:
                                        ...
                                        # SETTLING TIME
                                        data_dict=var_set_time(data_dict,i_time_event,data,stimulus_onset=stim_run[0][event])

                                        # REACTION TIME (maximum acceleration point)
                                        ...
                                        curve_output,slope_output=plot2pdf(pdf=pdf,question_table=question_table,
                                                 data_dict=data_dict,data_newton=data_newton,curve_output=curve_output,slope_output=slope_output,
                                        ...
                                        data_dict=nogos(data_dict,fbi_left_event,fbi_right_event,fbi_left_event_norm,
                                                        fbi_right_event_norm,i_time_event,idx_ant_onset,idx_stim_onset)
                                    ...
                                    data_dict = {key: (value + [numpy.nan] if 17 < key_idx < len_keylist else value) for key_idx, (key, value) in enumerate(data_dict.items())}
    ...
    print('Analysis finished. Preparing the group outputs...')
    ...
    print('Group data file saved')
    
    from scipy.io import savemat
    try:
        print("Saving force data...")
        savemat(os.path.join(savepath,'grips.mat'), curve_output, oned_as='row')
        print("Saving slope data...")
        savemat(os.path.join(savepath,'slopes.mat'), slope_output, oned_as='row')
    except:
        print('Failed to save grip data')
