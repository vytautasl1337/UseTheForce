...
def lowR(self,merged_data,left,right,times,list_of_blocks,runs,fig):
    ...
    fig.suptitle('Low reward grip responses (correct)', fontsize=20)
    ...
    for i in range(runs):
        ...
        try:
            for event in range(ant_run.size):
                if resp_run[0][event] == 1 and corr_run[0][event] == 1 and rew_run[event] == 'low ':
                ...
                elif resp_run[0][event] == -1 and corr_run[0][event] == 1 and rew_run[event] == 'low ':
                    idx_iti_onset = (numpy.abs(time_run - iti_run[0][event])).argmin()
                    idx_ant_onset = (numpy.abs(time_run - ant_run[0][event])).argmin()               
