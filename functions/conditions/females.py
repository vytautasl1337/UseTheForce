...
def on_females(self,merged_data,left,right,times,list_of_blocks,runs,fig):
    ...
    fig.suptitle('Grip responses to female faces (correct)', fontsize=20)
    ...
    for i in range(runs):
        ...
        dist_run = merged_data['Gender_of_Distractor'][i]
        resp_run = merged_data['GoNoGo_target'][i]
        try:
            for event in range(ant_run.size):
                if resp_run[0][event] == 1 and corr_run[0][event] == 1 and dist_run[0][event] == 2:
                ...
                elif resp_run[0][event] == -1 and corr_run[0][event] == 1 and dist_run[0][event] == 2:
                    idx_iti_onset = (numpy.abs(time_run - iti_run[0][event])).argmin()
                    idx_ant_onset = (numpy.abs(time_run - ant_run[0][event])).argmin()               
